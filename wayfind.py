...
class runApp(tk.Tk):
    def __init__(self):
        tk.Tk.__init__(self)
        self.fileNum = 0
        self.sessionStartTime = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        self.dataFilename = f'trajectory_{self.sessionStartTime}_filenum_{self.fileNum}.csv'
        
        # Initializing all other attributes
        self.floorIndex = 0
        self.pointIndex = 0
        self.actionCount = 0
        self.markerIndex = 0
        ...
        for img in IMG_FILES:
            self.imgList.append(ImageTk.PhotoImage(
                Image.open(r'images/'+img).resize((X, Y), Image.Resampling.LANCZOS)))
        # Frames
    ...
    def saveData(self):
        df = pd.DataFrame({
            'Time': self.timeList, 
            'Unix': self.unixList, 
            'Floor': self.floorList,
            'Marker': self.markerList,
            'X': self.xList, 
            'Y': self.yList
        }) 
        if not df.empty:
            df.to_csv(self.dataFilename, index=False)
            print(f'Data Saved as {self.dataFilename}')
        else:
    ...
    def autoSave(self):
        self.saveData()
        print(f'Data Saved. AutoSave checkpoint at {S_AUTOSAVE} seconds reached.')
        self.canvas.after(MS_AUTOSAVE, self.autoSave)
