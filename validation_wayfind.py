...
class runApp(tk.Tk):
    def __init__(self):
        ...
        # saves most recent absolute x position for the sign feature. Default is -1.
        self.xAbs = -1
        # saves most recent absolute y position for the sign feature. Default is -1.
        self.yAbs = -1
        ...
        for index, _ in enumerate(range(TASK_LIST_LENGTH)):
            self.taskBtnList.append('Task{}_Start'.format(index))
            self.taskBtnList.append('Task{}_End'.format(index))
        for index, _ in enumerate(range(SIGN_LIST_LENGTH)):
            self.signBtnList.append('Sign{}_Start'.format(index))
            self.signBtnList.append('Sign{}_End'.format(index))
        ...
        self.canvas.create_image(
            X/2, Y/2, anchor=tk.CENTER, image=self.imgList[0])
        ...
        self.msgList = []
        self.msgList.append(self.canvas.create_text(
            X/2, Y/2, text='', font=self.fnt))
        ...
        self.taskBtn = tk.Button(self.btnFrame, height=4*BTN_HEIGHT, width=BTN_WIDTH,
                                    text=self.taskBtnList[self.taskBtnIndex], command=partial(self.rotateMarkerBtn, 'taskBtn'))
        self.signBtn = tk.Button(self.btnFrame, height=4*BTN_HEIGHT, width=BTN_WIDTH,
                                    text=self.signBtnList[self.signBtnIndex], command=partial(self.rotateMarkerBtn, 'signBtn'))
        # Pack
        self.ascendBtn.pack(side = 'top')
        self.descendBtn.pack(side = 'top', pady = (0,BTN_SPACING))
        self.taskBtn.pack(side='top')
        self.signBtn.pack(side='top', pady = (0,BTN_SPACING))
        self.undoBtn.pack(side = 'top')
        self.redoBtn.pack(side = 'top')
    
    # is the current state of the checkbox
    def checkboxState(self):
      return map((lambda var: var.get()), self.vars)

    # opens data entry popup
    def popup(self, popupText):
        self.w = popupWindow(self, popupText)
    ...
    def rotateMarkerBtn(self, validButton):
        if validButton == 'taskBtn':
            self.btnMsg(str(self.taskBtnList[self.taskBtnIndex]))
            if (self.taskBtnIndex < len(self.taskBtnList)):
                self.taskBtnIndex += 1
            else:
                self.taskBtnIndex = 0
            self.taskBtn.config(text=str(self.taskBtnList[self.taskBtnIndex]))
        else:
            self.btnMsg(str(self.signBtnList[self.signBtnIndex]))
            if (self.signBtnIndex < len(self.signBtnList)):
                self.signBtnIndex += 1
            else:
                self.signBtnIndex = 0
            self.signBtn.config(text=str(self.signBtnList[self.signBtnIndex]))
        inVal = self.popup('Please enter a number.')
        self.addValid(inVal)
    ...
    def actionCounter(self, isPoint, isUndo=False):
        if isPoint:
            ...
            if self.actionCount >= PLOT_COUNT_MAX:
                self.actionCount -= 1
                self.canvas.delete(
                    self.plotList[self.pointIndex-PLOT_COUNT_MAX])
        else:
            if isUndo:
                self.pointIndex -= 1
                self.actionCount -= 1
        ...
        if isUndo:  # Undo
            if self.actionCount > 0:
                ...
                if xRel == 0 and yRel == 0:
                    ...
                    self.plotListMistake.append(
                        self.canvas.coords(self.plotList[-1]))
                    self.canvas.delete(self.plotList.pop())
    ...
    def saveData(self):
        ...
        df = pd.DataFrame({'Time': self.timeList, 'Unix': self.unixList, 'Floor': self.floorList,
                           'Validation Marker': self.markerList, 'X': self.xList, 'Y': self.yList})
        filename = ''
        if len(self.timeList) == 0:
            filename = 'empty.csv'
        else:
            filename = str(self.timeList[0])
            filename = filename[:filename.find(
                ' ')+9]+'-filenum-'+str(self.fileNum)+'.csv'
    ...
    def autoSave(self):
        self.saveData()
        print('Data Saved. AutoSave checkpoint at ' +
              str(S_AUTOSAVE)+' seconds reached.')
        self.canvas.after(MS_AUTOSAVE, self.autoSave)
...
class popupWindow(object):
    def __init__(self, master, pText):
        ...
        # value label
        self.l = tk.Label(top, text=pText)
...
if __name__ == "__main__":
    app = runApp()
    app.title('Wayfind: Real-World Wayfinding Tool - Validation Mode')
